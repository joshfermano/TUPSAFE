/**
 * Request Changes to PDS Submission API - POST /api/submissions/pds/[id]/request-changes
 *
 * Allows HR/admin to send a PDS submission back to the employee for revision.
...
import { db, pdsSubmissions, profiles, notifications } from '@tupsafe/database/server';
import { eq } from 'drizzle-orm';
import { requestChangesSchema } from '@tupsafe/types';
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    console.log('[Request Changes API] Request received');
    ...
    console.log('[Request Changes API] Requesting changes for submission:', submissionId);
    ...
    const [submission] = await db
      ...
      .where(eq(pdsSubmissions.id, submissionId))
      .limit(1);
    ...
    if (submission.status === 'approved') {
      return NextResponse.json(
        {
          error:
            'Cannot request changes for an approved submission. Consider creating a new version instead.',
        },
        { status: 400 }
      );
    }
    ...
    console.log('[Request Changes API] Submission updated to draft status');
    ...
    console.log('[Request Changes API] Audit log created');

    // Send notification to employee
    await db.insert(notifications).values({
      userId: submission.userId,
      type: 'submission_status',
      title: 'PDS Submission Requires Changes',
      message: `Your PDS submission (version ${submission.version}) requires changes. Please review the notes and resubmit. Notes: ${validatedData.notes}`,
      isRead: false,
    });

    console.log('[Request Changes API] Notification sent to employee');

    const totalDuration = Date.now() - startTime;
    console.log(
      `[Request Changes API] Request completed in ${totalDuration}ms`
    );
    ...
    console.error('[Request Changes API] Error:', error);
