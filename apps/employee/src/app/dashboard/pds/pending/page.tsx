...
export default function PDSPendingPage() {
  ...
  const pendingSubmissions = useMemo(() => {
    const pendingStatuses = ['draft', 'submitted', 'reviewing', 'rejected'];
      ...
      switch (sortBy) {
        ...
        case 'progress': {
          const progressA = calculateCompletion(
            a.status as 'draft' | 'submitted' | 'reviewing' | 'rejected'
          );
          const progressB = calculateCompletion(
            b.status as 'draft' | 'submitted' | 'reviewing' | 'rejected'
          );
  ...
  const stats = useMemo(() => {
    const draft = pendingSubmissions.filter((s) => s.status === 'draft').length;
