...
export function Liabilities() {
  ...
  const totalLiabilities = useMemo(() => {
    return liabilities.reduce(
      (sum: number, liability: LiabilityItem) => sum + (liability.outstandingBalance || 0),
      0
    );
  }, [liabilities]);
  ...
  return (
    <div className="space-y-6">
      <FormSection
        title="Liabilities"
        description="List all debts, loans, and financial obligations"
        icon={CreditCard}
      >
        <Alert className="mb-6">
          <Info className="h-4 w-4" />
          <AlertDescription>
            Include all outstanding debts such as home mortgages, car loans, personal loans, credit
            card balances, and business loans. If you have no debts, you may skip this section.
          </AlertDescription>
        </Alert>
        ...
        {fields.length === 0 ? (
          ...
          <div className="space-y-6">
            {fields.map((field, index) => (
              ...
              >
                ...
                <div className="grid gap-2">
                  <Label htmlFor={`liabilities.${index}.nature`} className="after:content-[&apos;*&apos;] after:ml-0.5 after:text-destructive">
                    Nature of Liability
                  </Label>
                  <Controller
                  ...
                  />
                  {errors?.liabilities && Array.isArray(errors.liabilities) && errors.liabilities[index] && (
                    <p className="text-sm text-destructive">
                      {(errors.liabilities[index] as LiabilityError)?.nature?.message}
                    </p>
                  )}
                </div>
                ...
                <div className="grid gap-2">
                  <Label htmlFor={`liabilities.${index}.creditorName`} className="after:content-[&apos;*&apos;] after:ml-0.5 after:text-destructive">
                  ...
                  {errors?.liabilities && Array.isArray(errors.liabilities) && errors.liabilities[index] && (
                    <p className="text-sm text-destructive">
                      {(errors.liabilities[index] as LiabilityError)?.creditorName?.message}
                    </p>
                  )}
                </div>
            ...
            <Button type="button" onClick={addLiability} variant="outline" className="w-full">
              <Plus className="h-4 w-4 mr-2" />
