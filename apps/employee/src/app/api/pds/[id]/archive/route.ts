...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (pds.submission.isLatest) {
      console.warn(
        `[POST /api/pds/[id]/archive] Archiving latest PDS ${id} for user ${session.user.id}. User will not have a current PDS.`
      );
    }

    // Archive PDS submission
    await archivePDSSubmission(id, session.user.id);

    console.log(
      `[POST /api/pds/[id]/archive] Archived PDS ${id} for user ${session.user.id}`
    );
