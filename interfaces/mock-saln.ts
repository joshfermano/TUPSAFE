...
export interface SalnSubmission {
  ...
  netWorth: number; // Computed field
...
export function getSalnRealPropertiesBySubmissionId(submissionId: string): SalnRealProperty[] {
  return mockSalnRealProperties.filter(prop => prop.salnSubmissionId === submissionId);
}

export function getSalnPersonalPropertiesBySubmissionId(submissionId: string): SalnPersonalProperty[] {
  return mockSalnPersonalProperties.filter(prop => prop.salnSubmissionId === submissionId);
}

export function getSalnLiabilitiesBySubmissionId(submissionId: string): SalnLiability[] {
  return mockSalnLiabilities.filter(liability => liability.salnSubmissionId === submissionId);
}

export function getSalnBusinessInterestsBySubmissionId(submissionId: string): SalnBusinessInterest[] {
  return mockSalnBusinessInterests.filter(business => business.salnSubmissionId === submissionId);
}

export function getSalnRelativesInGovBySubmissionId(submissionId: string): SalnRelativeInGov[] {
...
export const mockSalnDataSummary = {
  ...
  totalNetWorth: mockSalnSubmissions.reduce((sum, s) => sum + s.netWorth, 0),
  averageNetWorth: mockSalnSubmissions.reduce((sum, s) => sum + s.netWorth, 0) / mockSalnSubmissions.length,
};