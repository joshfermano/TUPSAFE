...
const passwordSchema = z
    ...
    .refine((password) => !password.includes('password') && !password.includes('123456'), 'Password must not contain common weak patterns');
...
const governmentEmailSchema = z
    ...
    return (GOVERNMENT_DEPARTMENTS.includes('Other Government Agency') ||
...
export const loginSchema = z.object({
    ...
    mfaCode: z
        .string()
        .optional()
        .refine((code) => !code || /^\d{6}$/.test(code), 'MFA code must be 6 digits'),
});
...
const baseRegisterSchema = z.object({
    ...
    phoneNumber: z
        ...
        .regex(/^(\+639|09)\d{9}$/, 'Please enter a valid Philippine phone number (e.g., +639123456789 or 09123456789)'),
    // Government Employment Details
    department: z
        .string()
        .min(1, 'Please select your department')
        .refine((dept) => GOVERNMENT_DEPARTMENTS.includes(dept), 'Please select a valid government department'),
    position: z
        .string()
    ...
    employeeId: z
        ...
        .regex(employeeIdRegex, 'Employee ID must be 3-15 characters (letters and numbers only)'),
    yearsOfService: z
        .number()
    ...
    dataProcessingConsent: z
        .boolean()
        .refine((val) => val === true, 'Consent for data processing is required for government compliance'),
});
